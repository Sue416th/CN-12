...
# Database only storage - no in-memory or file fallback
...
async def create_trip(request: TripCreateRequest):
    ...
    try:
        ...
        if request.profile:
            # Map price sensitivity string to float
            price_sensitivity_map = {
                "高": 0.8,
                "中等": 0.5,
                "低": 0.2
            }
            price_sensitivity_float = price_sensitivity_map.get(request.profile.price_sensitivity, 0.5)
            ...
            profile_context = {
                ...
                "price_sensitivity": price_sensitivity_float,
            }
        else:
            profile_context = {
                "user_id": user_id,
        ...
        if db_initialized:
            try:
                ...
                print(f"Error: Failed to save trip to MySQL: {e}")
                raise HTTPException(status_code=500, detail=f"Failed to save trip to database: {str(e)}")
        else:
        ...
        try:
            from src.agents.vector_service import get_vector_service
            vector_service = get_vector_service()
            vector_service.upsert_profile(user_id, profile_result.get("user_profile", {}))
            print(f"User profile saved to Milvus: {user_id}")
        except Exception as e:
            print(f"Warning: Failed to save profile to Milvus: {e}")
            # Continue without vector storage if Milvus is not available
...
async def list_trips(user_id: int = 1):
    ...
    if db_initialized:
        try:
            print(f"===== list_trips started =====")
            print(f"Calling get_user_trips with user_id: {user_id}")
            user_trips = await get_user_trips(user_id)
            print(f"Got user_trips: {len(user_trips)} trips")
            # Convert to frontend format
            for trip in user_trips:
                print(f"Processing trip: {trip.get('id')}")
                ...
                print(f"Itinerary type: {type(trip.get('itinerary'))}, Itinerary value: {trip.get('itinerary')}")
                trip["profile"] = trip.get("itinerary", {}).get("profile", {}) if trip.get("itinerary") else {}
            print(f"===== list_trips ended =====")
            return {
                "success": True,
                "trips": user_trips,
            ...
            import traceback
            print(f"Error: Failed to get trips from MySQL: {e}")
            print(f"Traceback: {traceback.format_exc()}")
            raise HTTPException(status_code=500, detail=f"Failed to get trips from database: {str(e)}")
    else:
        raise HTTPException(status_code=500, detail="Database not initialized")
...
async def get_trip_detail(trip_id: str):
    ...
    if db_initialized:
        try:
            ...
            print(f"Error: Failed to get trip from MySQL: {e}")
            raise HTTPException(status_code=500, detail=f"Failed to get trip from database: {str(e)}")
    else:
        raise HTTPException(status_code=500, detail="Database not initialized")
        ...
        try:
            ...
            if trip:
                ...
                try:
                    ...
                    print(f"Error: Failed to update trip in MySQL: {e}")
                    raise HTTPException(status_code=500, detail=f"Failed to update trip in database: {str(e)}")
            else:
                raise HTTPException(status_code=404, detail="Trip not found")
        except HTTPException:
            raise
...
async def delete_trip(trip_id: str):
    ...
    if db_initialized:
        try:
            ...
            print(f"Error: Failed to delete trip from MySQL: {e}")
            raise HTTPException(status_code=500, detail=f"Failed to delete trip from database: {str(e)}")
    else:
        raise HTTPException(status_code=500, detail="Database not initialized")
...
async def analyze_profile(request: UserProfileRequest):
    ...
    try:
        ...
        price_sensitivity_float = price_sensitivity_map.get(request.price_sensitivity, 0.5)
        ...
        try:
            from src.agents.vector_service import get_vector_service
            vector_service = get_vector_service()
            vector_service.upsert_profile(request.user_id, profile_result.get("user_profile", {}))
            print(f"User profile saved to Milvus: {request.user_id}")
        except Exception as e:
            print(f"Warning: Failed to save profile to Milvus: {e}")
...
async def get_user_profile(user_id: int = 1):
    """
    Get user profile from database and vector database
    """
    try:
        # Get from MySQL
        from src.db import get_user_profile as db_get_user_profile
        profile = await db_get_user_profile(user_id)
        
        # Also try to get from vector database
        vector_profile = None
        try:
            from src.agents.vector_service import get_vector_service
            vector_service = get_vector_service()
            # For now, we'll just return the MySQL profile
            # In the future, we could enhance this to include vector-based insights
        except Exception as e:
            print(f"Warning: Failed to get profile from Milvus: {e}")
