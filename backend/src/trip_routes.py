...
from src.service.navigation import get_route
from src.service.poi_search import search_poi
...
async def get_navigation_route(start: str, end: str, mode: str = "driving"):
    """
...
@router.get("/poi")
async def search_points_of_interest(keyword: str, city: str, types: str = None, radius: int = 1000):
    """
    Search points of interest in a city
    """
    try:
        # 调用POI搜索功能
        result = search_poi(keyword, city, types, radius)
        return {"pois": result}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
