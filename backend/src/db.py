...
async def get_trip(trip_id: str) -> Optional[Dict[str, Any]]:
    ...
    if row and row.get("itinerary") and isinstance(row["itinerary"], str):
...
async def get_user_trips(user_id: int) -> List[Dict[str, Any]]:
    ...
    print(f"Found {len(rows)} trips for user {user_id}")
    
    trips = []
    for row in rows:
        print(f"Trip ID: {row.get('id')}, Itinerary type: {type(row.get('itinerary'))}, Itinerary value: {row.get('itinerary')}")
        # 检查itinerary字段的类型，如果是字符串则解析
        if isinstance(row.get('itinerary'), str):
            try:
                row['itinerary'] = json.loads(row['itinerary'])
                print(f"After parsing, itinerary type: {type(row.get('itinerary'))}")
